...
export default function EditorialAdminPage() {
  ...
  const refresh = () => queryClient.invalidateQueries({ queryKey: ['editorial-admin'] });
          ...
          <div className="mt-6 space-y-4">
            {(query.data?.pendingApplications ?? []).map((application) => (
              <article key={application.id} className="rounded-2xl border border-white/10 bg-white/[0.04] p-5">
                ...
                {application.experience ? <p className="mt-3 text-sm text-white/45"><strong>Experience:</strong> {application.experience}</p> : null}
                {application.portfolioUrl ? <a href={application.portfolioUrl} target="_blank" rel="noreferrer" className="mt-3 inline-block text-sm text-sky-400">View portfolio ↗</a> : null}
                <textarea value={notes[`writer-${application.id}`] ?? ''} onChange={(e) => setNotes({ ...notes, [`writer-${application.id}`]: e.target.value })} placeholder="Optional review notes" className="mt-4 w-full rounded-xl border border-white/10 bg-white/5 p-3 text-sm text-white outline-none" />
                <div className="mt-3 flex gap-2">
                  ...
                  <button disabled={writerMutation.isPending} onClick={() => writerMutation.mutate({ id: application.id, status: 'rejected' })} className={`${button} bg-red-500/15 text-red-300`}>Reject</button>
                </div>
              </article>
            ))}
