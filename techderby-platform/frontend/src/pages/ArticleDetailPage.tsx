      ...
      <Container className="max-w-3xl">
        <article className="pb-24">
          ...
          <div className="mb-10 flex items-center justify-between gap-4 border-b border-white/[0.07] pb-8">
            <div className="flex items-center gap-3">
              {article.authorAvatar ? (
                <img src={article.authorAvatar} alt={article.authorName ?? undefined} className="h-10 w-10 rounded-full object-cover" />
              ) : (
          ...
          <div className="mt-10 rounded-2xl border border-white/[0.07] bg-white/[0.02] p-6 flex items-start gap-4">
            {article.authorAvatar ? (
              <img src={article.authorAvatar} alt={article.authorName ?? undefined} className="h-14 w-14 rounded-xl object-cover shrink-0" />
            ) : (
              ...
              <div className="mb-8 flex gap-3">
                {user.avatar ? (
                  ...
                  <div className="flex h-9 w-9 shrink-0 mt-0.5 items-center justify-center rounded-full bg-gradient-to-br from-sky-500 to-indigo-500 text-xs font-black text-white">
                    {([user.first_name, user.last_name].filter(Boolean).join(' ') || user.username)[0].toUpperCase()}
                  </div>
                )}
              ...
              <ul className="space-y-5">
                {comments.map((c) => (
                  <li key={c.id} className="flex gap-3">
                    {c.authorAvatar ? (
                      <img src={c.authorAvatar} alt={c.authorName ?? undefined} className="h-8 w-8 rounded-full object-cover shrink-0 mt-0.5" />
                    ) : (
                    ...
                    {user && (user.id === c.authorId || ['admin', 'super-admin'].includes(user.member_role)) && (
                      <button
                        type="button"
