...
function UserAvatar({ user }: { user: AdminUser }) {
  ...
  const role = (user.member_role ?? 'member') as MemberRole;
...
// ── CreateUserModal ───────────────────────────────────────────────────────────
const BLANK_FORM = { first_name: '', last_name: '', username: '', email: '', password: '', member_role: 'member' as MemberRole };
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                autoComplete="off"
              />
              {errors.first_name && <p className="mt-1 text-[11px] text-red-400">{errors.first_name}</p>}
            </div>
            <div>
              ...
              <input
                ...
                autoComplete="off"
              />
              {errors.last_name && <p className="mt-1 text-[11px] text-red-400">{errors.last_name}</p>}
            </div>
          </div>
...
export default function AdminUsersPage() {
  ...
  const handleRoleSelect = useCallback((user: AdminUser, role: MemberRole) => {
    if (role === user.member_role) return;
