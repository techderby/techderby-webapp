...
const NAV_AUTHOR: NavItem[] = [
  {
    to: '/dashboard/author/articles', label: 'My Articles',
    icon: <svg viewBox="0 0 24 24" className="h-[18px] w-[18px]" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /><line x1="16" y1="13" x2="8" y2="13" /><line x1="16" y1="17" x2="8" y2="17" /><polyline points="10 9 9 9 8 9" /></svg>,
  },
  {
    to: '/dashboard/author/articles/new', label: 'New Article',
    icon: <svg viewBox="0 0 24 24" className="h-[18px] w-[18px]" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 20h9" /><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z" /></svg>,
  },
  {
    to: '/dashboard/admin/articles', label: 'Review Articles',
    icon: <svg viewBox="0 0 24 24" className="h-[18px] w-[18px]" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 11l3 3L22 4" /><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11" /></svg>,
  },
];

// Admin-only nav (hub, author management, user management)
const NAV_ADMIN: NavItem[] = [
  {
    to: '/dashboard/admin/hub', label: 'Admin Hub',
    icon: <svg viewBox="0 0 24 24" className="h-[18px] w-[18px]" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/></svg>,
  },
  {
    to: '/dashboard/admin/authors', label: 'Authors',
    icon: <svg viewBox="0 0 24 24" className="h-[18px] w-[18px]" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" /><circle cx="9" cy="7" r="4" /><line x1="23" y1="11" x2="17" y2="11" /><line x1="20" y1="8" x2="20" y2="14" /></svg>,
  },
  {
  ...
  return (
    <div className="flex h-full flex-col bg-[#07090f]">
      ...
      <nav className={cn('flex-1 overflow-y-auto pb-2 scrollbar-none', collapsed ? 'mt-1 px-[11px]' : 'mt-3 px-3')}>
        ...
        {/* ── Writing section (authors) ── */}
        {['editor', 'admin', 'super-admin'].includes(role) && (
          <>
            <div className={cn('my-3 border-t border-white/[0.06]', collapsed ? 'mx-0' : 'mx-2')} />
            {!collapsed && (
              <p className="mb-1.5 px-3 text-[9px] font-black uppercase tracking-[0.2em] text-white/[0.18]">Writing</p>
            )}
            {NAV_AUTHOR.map((item) =>
              collapsed ? (
                <Tip key={item.to} label={item.label}>
                ...
                <NavLink
                  ...
                  className={({ isActive }) => cn(
                    'group relative mb-0.5 flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-all duration-150 select-none',
                    isActive
                      ? 'bg-gradient-to-r from-emerald-500/[0.14] to-transparent text-emerald-300'
                      : 'text-white/40 hover:bg-white/[0.04] hover:text-white/85',
                  )}
                >
                  {({ isActive }) => (
                    <>
                      <span className={cn(
                        'absolute left-0 top-1/2 -translate-y-1/2 w-[3px] rounded-r-full bg-gradient-to-b from-emerald-400 to-teal-400 transition-all duration-200',
                        isActive ? 'h-5 opacity-100' : 'h-0 opacity-0',
                      )} />
                      <span className={cn('shrink-0 transition-colors duration-150', isActive ? 'text-emerald-400' : 'text-white/25 group-hover:text-white/55')}>
                        {item.icon}
                      </span>
                      <span className="truncate">{item.label}</span>
                    </>
        ...
        {/* ── Become an Author (members only) ── */}
        {role === 'member' && (
          <>
            <div className={cn('my-3 border-t border-white/[0.06]', collapsed ? 'mx-0' : 'mx-2')} />
            {collapsed ? (
              <Tip label="Become an Author">
                <NavLink
                  to="/dashboard/author/apply"
                  onClick={onClose}
                  className={({ isActive }) => cn(
                    'mb-0.5 flex h-10 w-10 items-center justify-center rounded-xl transition-all duration-150',
                    isActive ? 'bg-sky-500/20 text-sky-400' : 'text-white/30 hover:bg-white/6 hover:text-white/75',
                  )}
                >
                  <svg viewBox="0 0 24 24" className="h-[18px] w-[18px]" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/><line x1="19" y1="11" x2="23" y2="11" /><line x1="21" y1="9" x2="21" y2="13" />
                  </svg>
                </NavLink>
              </Tip>
            ) : (
              <NavLink
                ...
                onClick={onClose}
                className={({ isActive }) => cn(
                  'group relative mb-0.5 flex items-center gap-3 rounded-xl border px-3 py-2.5 text-sm font-medium transition-all duration-150',
                  isActive
                    ? 'border-sky-500/20 bg-sky-500/10 text-sky-300'
                    : 'border-white/[0.05] bg-white/[0.015] text-white/50 hover:border-white/10 hover:bg-white/[0.04] hover:text-white/80',
                )}
              >
                {({ isActive }) => (
                  <>
                    <span className={cn('shrink-0', isActive ? 'text-sky-400' : 'text-white/25 group-hover:text-white/55')}>
                      <svg viewBox="0 0 24 24" className="h-[18px] w-[18px]" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                        <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/><line x1="19" y1="11" x2="23" y2="11" /><line x1="21" y1="9" x2="21" y2="13" />
                      </svg>
                    </span>
                    <span className="truncate">Become an Author</span>
                    <span className="ml-auto rounded-full border border-sky-500/25 bg-sky-500/10 px-1.5 py-0.5 text-[9px] font-black uppercase tracking-wide text-sky-400">New</span>
                  </>
                )}
              </NavLink>
            )}
        ...
        {/* ── Editorial section (admins) ── */}
        {['admin', 'super-admin'].includes(role) && (
          <>
            <div className={cn('my-3 border-t border-white/[0.06]', collapsed ? 'mx-0' : 'mx-2')} />
            {!collapsed && (
              <p className="mb-1.5 px-3 text-[9px] font-black uppercase tracking-[0.2em] text-white/[0.18]">Editorial</p>
            )}
            {NAV_ADMIN.map((item) =>
              collapsed ? (
                <Tip key={item.to} label={item.label}>
                  <NavLink
                    ...
                    className={({ isActive }) => cn(
                      'mb-0.5 flex h-10 w-10 items-center justify-center rounded-xl transition-all duration-150',
                      isActive
                        ? 'bg-purple-500/20 text-purple-400 ring-1 ring-purple-500/25'
                        : 'text-white/30 hover:bg-white/6 hover:text-white/75',
                    )}
                  >
                    {item.icon}
                ...
                <NavLink
                  ...
                  className={({ isActive }) => cn(
                    'group relative mb-0.5 flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-all duration-150 select-none',
                    isActive
                      ? 'bg-gradient-to-r from-purple-500/[0.14] to-transparent text-purple-300'
                      : 'text-white/40 hover:bg-white/[0.04] hover:text-white/85',
                  )}
                >
                  {({ isActive }) => (
                    <>
                      <span className={cn(
                        'absolute left-0 top-1/2 -translate-y-1/2 w-[3px] rounded-r-full bg-gradient-to-b from-purple-400 to-indigo-400 transition-all duration-200',
                        isActive ? 'h-5 opacity-100' : 'h-0 opacity-0',
                      )} />
                      <span className={cn('shrink-0 transition-colors duration-150', isActive ? 'text-purple-400' : 'text-white/25 group-hover:text-white/55')}>
                        {item.icon}
                      </span>
                      <span className="truncate">{item.label}</span>
                    </>
